...
type ProjectFeedScreenProps = NativeStackScreenProps<MainStackParamList, 'ProjectFeed'>;

export const ProjectFeedScreen: React.FC<ProjectFeedScreenProps> = ({ route }) => {
    ...
    return (
        ...
        >
            <FlatList
                ...
                inverted // Show newest at bottom (standard chat behavior, but requires data reversal if API returns oldest first)
                // Note: If API returns oldest first, we might need to reverse data or not use inverted. 
                // Assuming API returns newest first for now, or we handle sorting.
                // Actually, standard is API returns newest first for pagination, but for chat we usually want oldest at top if not inverted, or newest at bottom.
                // Let's assume we want to scroll to bottom.
